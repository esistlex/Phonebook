...
namespace phonebook1
{
    public partial class PhonebookForm : Connector
    {
        ...
        private void load()
        {
            ...
            try
            {
                ...
                while (reader.Read())
                {
                    ...
                    row.CreateCells(dataGridView1, getID, getName, getNumber, getAddress, getProvince, getZipcode, getCountry);
        ...
        private void searchBtnClick(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Cannot search, please try again!");
            } else
            {
                try
                {
                    DoConnect();
                    String query = "SELECT * FROM contactlist WHERE USER_NAME = @Name";
                    command = new MySqlCommand(query, connection);
        ...
        private void addBtnClick(object sender, EventArgs e)
        {
            ...
            if (nameTxtbox.TextLength == 0 && contactTxtbox.TextLength == 0 && addressTxtbox.TextLength == 0  && provinceTxtbox.TextLength == 0 && zipcodeTxtbox.TextLength == 0 && countryTxtbox.TextLength == 0)
            {
            ...
            else
            {
                ...
                {
                    DoConnect();
                    String query = "INSERT INTO contactlist (USER_NAME, USER_NUMBER, USER_ADDRESS, USER_STATE, USER_ZIPCODE, USER_COUNTRY) VALUES (@Name, @Number, @Address, @State, @ZipCode, @Country)";
                    command = new MySqlCommand(query, connection);
        ...
        private void deleteBtnClick(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    DoConnect();
                    String query = "DELETE FROM contactlist WHERE USER_NAME = @Name AND USER_NUMBER = @Number AND USER_ADDRESS = @Address AND USER_STATE = @State AND USER_ZIPCODE = @ZipCode AND USER_COUNTRY = @Country";
                    command = new MySqlCommand(query, connection);
                    command.Parameters.AddWithValue("@Name", nameTxtbox.Text);
                    command.Parameters.AddWithValue("@Number", contactTxtbox.Text);
                    command.Parameters.AddWithValue("@Address", addressTxtbox.Text);
        ...
        private void updateBtnClick(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    DoConnect();
                    String query = "UPDATE contactlist SET USER_NAME = @Name, USER_NUMBER = @Number, USER_ADDRESS = @Address, USER_STATE = @State, USER_ZIPCODE = @ZipCode, USER_COUNTRY = @Country WHERE USERID = @Id";
                    command = new MySqlCommand(query, connection);
